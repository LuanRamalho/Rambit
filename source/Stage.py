...
class Stage(BaseObject.BaseObject):
    ...
    def drawMap(self):
        ...
        # Cálculos para o início e fim da câmera, convertendo corretamente para inteiros
        camRow = int(self.screenManager.camera.y / self.tileHeight)
        camCol = int(self.screenManager.camera.x / self.tileWidth)
        camEndRow = int((self.screenManager.camera.y + self.screenManager.resolution[1]) / self.tileHeight)
        camEndCol = int((self.screenManager.camera.x + self.screenManager.resolution[0]) / self.tileWidth)

        # Iterando sobre a área visível da câmera
        for row in range(camRow, camEndRow + 1):
            for col in range(camCol, camEndCol + 1):
                # Verifica se não está fora de índice
                if row >= 0 and col >= 0 and row < self.mapDic['mh'] and col < self.mapDic['mw']:
                    # Posição do tile na lista de tiles 'tileList'
    ...
    def validateDx(self, dx, gameObject, previousPositionY):
        # Certifique-se de que as variáveis sejam inteiras
        row_ = int((gameObject.position.y + gameObject.nonHitArea[1]) / self.tileHeight)
        col_ = int((previousPositionY + gameObject.nonHitArea[0]) / self.tileWidth)
        endRow = int((gameObject.position.y + gameObject.size.y - gameObject.nonHitArea[3]) / self.tileHeight)
        endCol = int((previousPositionY + gameObject.size.x - gameObject.nonHitArea[2]) / self.tileWidth)

        for row in range(row_, endRow + 1):
            for col in range(col_, endCol + 1):
                # Verifica se não está fora de índice
                if row >= 0 and col >= 0 and row < self.mapDic['mh'] and col < self.mapDic['mw']:
                    # O valor do tile, o número que indica aquele tile
                    tileNum = self.mapDic['tl'][row][col]
                    # Verifica se é um tile que pode ser colidido
                    if self.canCollide(tileNum):
                        blockPosX = col * self.tileWidth
                        blockPosY = row * self.tileHeight
                        # Posição do gameObject, mantendo o eixo y
                        ...
                        if goRect.colliderect(blockRect):
                            if dx > 0:
                                # Se deslocou para a direita e colidiu, volta o personagem para a esquerda
                                gameObject.position.x = blockPosX - gameObject.size.x + gameObject.nonHitArea[2]
                            elif dx < 0:
                                # Se deslocou para a esquerda e colidiu, volta o personagem para a direita
    ...
    def validateDy(self, dy, gameObject, previousPositionX):
        row_ = int((gameObject.position.y + gameObject.nonHitArea[1]) / self.tileHeight)
        col_ = int((previousPositionX + gameObject.nonHitArea[0]) / self.tileWidth)
        endRow = int((gameObject.position.y + gameObject.size.y - gameObject.nonHitArea[3]) / self.tileHeight)
        endCol = int((previousPositionX + gameObject.size.x - gameObject.nonHitArea[2]) / self.tileWidth)
